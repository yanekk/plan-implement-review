...
| T18 | live-run | T08, T13, T14, T16, T17 | ✅ | Review: one fix, human-flow.md still said `y` approves. Unrecorded seatbelt raise (fixture ceiling 2, 15 min) kept by the user, written into §5.3. Hand half ✅ (FINDINGS). Unverified: the final fixture never completed a harness run (user stopped it); T04's `go` path is unit-tested only. Probed picker, gate, background count, answerer idle timing. |

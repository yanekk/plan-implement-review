...
Nothing yet. Two `you` tasks need the user when they are ready: the T00 spike (early, drives the
messaging-mode probe) and the T07 capstone (last, the live end-to-end drill). Both are bounded —
scratch sessions and a scratch plan at ceiling 1, torn down after (DESIGN §5.2). Each carries its
own "Needs a person" block; under `pir-coordinate` the hands-on worker presents it and waits.

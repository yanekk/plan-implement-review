...
- Reuses `src/shell/identity.mjs` (T05), `src/shell/index-store.mjs` (T06),
  `src/shell/snapshot-store.mjs` (T07) for removing the snapshot, and the worker-reap primitives in
  `src/shell/platform.mjs` (`list`, `close`) with `src/core/naming.mjs` (`isWorkerOf`).

## Interface

```
stopRun(record, { kill, exec, now, sleep, graceMs = 4000, fs } = {}) →
  ...
  //    (platform.list filtered by isWorkerOf(record.slug)) and close each — escalated:true.
...
injected so tests drive the timeline without real signals.
...
- [ ] escalated reap closes only workers matching `isWorkerOf(slug)`, not another run's.

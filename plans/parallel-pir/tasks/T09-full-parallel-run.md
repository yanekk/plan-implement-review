# T09 — Full multi-worker run + kill-switch drill

**Phase:** 3 · **Depends on:** T08 · **Weight:** heavy
...
serially to `main`, with a worker deliberately raising a decision to prove the surfacing works
under load, and a kill-switch drill partway through to prove the hard stop tears every worker
down and that restart-from-`PROGRESS.md` resumes cleanly. Nothing new is built beyond a real
multi-task scratch plan; the task is the verification that the whole mode works for real.

## Design sections this implements

DESIGN §2.1 (concurrent build and review), §2.4 (the ceiling and kill switch under real load),
§2.5 (a worker blocked on a decision, serialized merge, kill switch mid-task), §6 (recovery).
This is the §5.1 hand-verified row for the full run.
...
No new interface. Exercises `pir-coordinate` (T08) over the real platform with the ceiling at 4
and several independent tasks, one of which raises a decision.

## Tests

The automated suite is already green from T01–T08. This task adds no unit tests unless the run
...
- [ ] A real small plan of several independent tasks was run to all-`✅` with multiple workers
      live at once, ceiling respected, merges serialized, one decision surfaced and answered,
...
a decision surfaced to you while the rest keep moving; on HALT, all workers stopped and nothing
half-merged; on restart, the run resumes from `PROGRESS.md` and finishes to all-`✅`.
Tell me: did the ceiling hold, did merges stay one-at-a-time, was the decision surfaced without
stalling the others, did HALT tear everything down cleanly, and did restart resume without
redoing finished work or corrupting `main`.

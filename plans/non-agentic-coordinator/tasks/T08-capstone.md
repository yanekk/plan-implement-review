# T08 — Live end-to-end hand-verification with the person

**Phase:** 4 · **Runs:** you · **Depends on:** T05, T07 · **Weight:** light (gated on the person)
...
person can find and answer a blocked worker in `claude agents`, that Ctrl-C kills real workers and a
re-run resumes, or that a worker's own git/test clears the classifier without a prompt. This task
sets all that up and puts the running thing in front of the person to judge.

## Design sections this implements

DESIGN §5.1 (what the test command cannot reach) and §2.3, §2.2, §2.6, §2.4 confirmed live.
...
```
# bring-up: a throwaway clone and a lean scratch plan (a few trivial auto tasks, one that asks a
# question), marked reviewed so the command will run.
git clone . ../pir-scratch
# seed plans/scratch/ in ../pir-scratch: PROGRESS.md (reviewed), a few tiny tasks, deps giving 2–3
# concurrent, one task whose doc asks a genuine question.
...
reviewing, merged, with the ceiling and elapsed times; one task parks as `asking you`; answering it
(find it in `claude agents`, attach, reply) un-parks it; Ctrl-C closes the workers and leaves the
scratch `main` untouched; re-running resumes from committed work; the run ends by printing the green
branch and `git merge`; a worker committed without a classifier prompt.

Tell me:
- Does the live display read right in a real terminal (in place, legible, the right statuses)?
- Did answering a blocked worker directly, then Ctrl-C, then a re-run, behave as described?
- Did a worker's own `git commit`/`npm test` run without a permission prompt (T05 working live)?
...
- [ ] The person confirms the live display reads right, direct answer + Ctrl-C + re-run behaved as

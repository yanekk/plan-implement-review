...
| 2026-09-19 | 📌 | Naming keeps the `·` separator. A `/` inside a name survives `parseAgentName` (it splits on `·`), but reverting the separator to `/` is churn plus an unverified `claude --bg -n` launch path; the reason for `·` (SendMessage) is gone but `·` still works. |

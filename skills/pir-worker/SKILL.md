...
implemented** (`kind: implemented`) and stop. The coordinator closes your implement session and spawns
...
- **On a merge conflict in code:** attempt the resolution yourself — you hold this task's context. If
  you cannot resolve it cleanly, message the coordinator (`kind: conflict`) with the conflicting files
  and wait. Never hand back a dirty branch.
- When it integrates cleanly, message the coordinator (`kind: done`).
...
branch, mark the task done, and message the coordinator (`kind: done`). You produce no code and get no
